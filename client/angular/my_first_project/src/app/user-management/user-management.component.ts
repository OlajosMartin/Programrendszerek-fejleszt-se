...
export class UserManagementComponent {
        ...
        if(data){
          ...
          this.userService.delete(id).subscribe({
            next: (data) => {
              ...
              this.openSnackBar('A felhasználó sikeresen törölve lett.', 3000)
            }, error: (err) => {
              console.log(err);
            }
          });
  ...
  navigateToUploadBooking(){
    this.router.navigateByUrl('/bookings');
  }

  navigateToBookings(){
    this.router.navigateByUrl('/bookings-list');
  }

  navigateToOffers(){
    this.router.navigateByUrl('/all-offer');
  }
